...
export const signUp: RequestHandler<unknown, unknown, SignUpBody, unknown> = async (req, res, next) => {
  ...
  try {
    ...
    const uploadedUserPhotoUrl = userPhotoUrl ? await uploadImage(userPhotoUrl) : '';
    ...
    const newUser = await UserModel.create({
      ...
      role: userRoleValue, 
      userPhotoUrl: uploadedUserPhotoUrl, 
...
export const updateUser: RequestHandler<unknown, unknown, UpdateUserBody, unknown> = async (req, res, next) => {
  ...
  try {
    const currentUser = await UserModel.findById(userToUpdate._id);

    if(!currentUser) {
      throw(createHttpError(400, `Cannot find a user with such ID: ${userToUpdate._id}`));
    }
    ...
    const userPhotoUrl = userToUpdate && userToUpdate.userPhotoUrl && userToUpdate.userPhotoUrl !== currentUser.userPhotoUrl ? 
      currentUser.userPhotoUrl ? 
        await updateImage(userToUpdate.userPhotoUrl, currentUser!.userPhotoUrl!) : 
        await uploadImage(userToUpdate.userPhotoUrl) : 
      currentUser.userPhotoUrl;

    await UserModel.findByIdAndUpdate(userToUpdate._id, { ...userToUpdate, userPhotoUrl });
...
export const deleteUser: RequestHandler = async (req, res, next) => {
  ...
  try {
    ...
    if(user && user.userPhotoUrl) {
      await deleteImage(user.userPhotoUrl);
    }
